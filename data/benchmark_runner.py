...
def run_benchmark(config, output_folder, run_number):
    ...
    cmd = [
        ...
        # save request rate, temperature, dataset, distribution to a json file
...
def save_results(outputs, config, config_file, output_folder):
    ...
    benchmark_params = config['test']['benchmark']
    
    with open(output_file, 'w') as f:
        f.write(f"Benchmark Results: {config['test']['name']}\n")
...
def main():
        ...
        for run in range(1, runs + 1):
            ...
            try:
                ...
                if not wait_for_server():
                    print("Skipping this run due to server startup failur               e")
                    continue
    ...
    # Save all results
    if outputs:
        save_results(outputs, config, args.config, args.output)
        print(f"\nCompleted {len(outputs)}/{args.runs} successful runs")
    else:
